      ...
      <div className="flex flex-wrap gap-3">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {specializations.map((spec) => (
            <Button
              key={spec}
              variant={search === spec ? 'default' : 'outline'}
              size="sm"
      ...
      {filtered.length === 0 ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map((doctor) => (
            <Card
              key={doctor.id}
              className="stat-card-shadow transition-all hover:stat-card-shadow-hover hover:-translate-y-0.5"
            >
              <CardContent className="p-5">
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <Badge variant="secondary" className="mt-1">
                      {doctor.doctor?.specialization || 'Généraliste'}
                    </Badge>
                    {doctor.doctor?.office_number && (
                      <p className="mt-2 flex items-center gap-1 text-xs text-muted-foreground">
                        <MapPin className="h-3 w-3" />
                        Bureau {doctor.doctor.office_number}
                      </p>
                    )}
                  </div>
                </div>
                <Button
                  ...
                  onClick={() => navigate('/patient/book', { state: { doctorId: doctor.id } })}
                >
                  Prendre rendez-vous
                </Button>
              </CardContent>
