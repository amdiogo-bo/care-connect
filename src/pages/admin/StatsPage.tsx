...
const AdminStatsPage = () => {
  const stats = useMemo(() => {
    ...
    return { total, completed, rate, monthly, specializations: Object.entries(bySpec).map(([name, value]) => ({ name, value })), roleBreakdown };
  }, []);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {[
          { label: 'Utilisateurs', value: mockUsers.length, icon: Users, color: 'text-primary' },
          { label: 'Médecins', value: mockDoctors.length, icon: Stethoscope, color: 'text-accent' },
          { label: 'Total RDV', value: stats.total, icon: Calendar, color: 'text-warning' },
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <Card><CardHeader><CardTitle>Utilisateurs par rôle</CardTitle></CardHeader><CardContent>
          <ResponsiveContainer width="100%" height={250}>
            <PieChart><Pie data={stats.roleBreakdown} cx="50%" cy="50%" outerRadius={80} dataKey="value" label={({name, value}) => `${name}: ${value}`}>
              {stats.roleBreakdown.map((_, i) => <Cell key={i} fill={COLORS[i % COLORS.length]} />)}
        ...
        <Card><CardHeader><CardTitle>Spécialités médicales</CardTitle></CardHeader><CardContent>
          <ResponsiveContainer width="100%" height={250}>
            <PieChart><Pie data={stats.specializations} cx="50%" cy="50%" outerRadius={80} dataKey="value" label={({name, value}) => `${name}: ${value}`}>
              {stats.specializations.map((_, i) => <Cell key={i} fill={COLORS[i % COLORS.length]} />)}
            </Pie><Tooltip /></PieChart>
          </ResponsiveContainer>
        </CardContent></Card>
      </div>
